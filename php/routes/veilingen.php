<?php
    ...
    add_route('GET', 'veiling\/(?<voorwerpnummer>[0-9]+)\/update', function($voorwerpnummer) {
        $veiling = get_veiling($voorwerpnummer);
        ...
        {
            $result = [
                'bod' => $veiling['minimalBod'],
                'hoogsteBod' => '&euro; ' . $veiling['verkoopPrijs'],
                'hoogsteBieder' => $veiling['koper']
            ];
        }
    ...
    add_route('POST', 'veiling\/create\/formulier', function(){
        ...
        if($_SERVER['HTTP_REFERER'] == get_url(true) .  'veiling/create/formulier'){ 
            ...
            if(empty($errors))
            {
                ...
                location('veiling/' . $veilingstart);
            }

            set_data_view('errors', $errors);
        }
        else if (empty($_POST['rubrieknummer']))
        {
            location('veiling/create');
            return;
        }
